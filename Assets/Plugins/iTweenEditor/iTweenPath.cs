...
public class iTweenPath : MonoBehaviour
{
	...
	public List<Vector3> offset = new List<Vector3>(){Vector3.zero, Vector3.zero};
	...
	public void Start(){
		...
		foreach(Vector3 vec in nodes){
			offset.Add(vec - transform.parent.transform.position);
		}
	}

	public void Update(){
		for(int i = 0; i < nodes.Count; i++){
			nodes[i] = transform.parent.transform.position + offset[i];
		}
	}
